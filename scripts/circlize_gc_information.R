...
# modified script with hardcoded IIC genome because couldn't get the options to work
# Original content from Daniel Giguere at https://github.com/dgiguer/complete-and-validated-genomes-from-a-metagenome repository for creating a genome track in Circlize

library(optparse)
library(seqinr)

# keep as default foramt for now.
fasta <- read.fasta(file = "results/genome_track/UW6-oriented.fasta")  
...
fasta.string <- read.fasta(file = "results/genome_track/UW6-oriented.fasta", as.string=TRUE)  
...
write.table(final_gc_info, file = "results/genome_track/gc_calculations_oriented.txt", col.names=TRUE, row.names=FALSE, quote = FALSE)
